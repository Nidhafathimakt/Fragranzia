...
function SideBar({ open, setOpen }) {
  const link = ({
    isActive,
  }) => `px-6 py-4 text-xl font-semibold border-b border-white/20 flex items-center gap-2 
  ${
    isActive
  ...
  return (
  <div
  className={`fixed top-0 left-0 z-50 h-screen w-70 sidebarItem overflow-y-auto transition-transform duration-300
  ${open ? "translate-x-0" : "-translate-x-full"}
  lg:translate-x-0`}
>
      <div className="sidebarItem flex items-center justify-between text-white px-6 py-8 border-b border-white/20  ">
        <h2 className="text-2xl font-semibold ">Fragranzia</h2>
