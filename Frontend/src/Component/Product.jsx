...
function Product() {
  const { data, addToCart, goToDetails,  } =
        ...
        <div className="mt-6 px-6 md:px-12">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex flex-col md:flex-row gap-2 md:gap-6 text-sm md:text-lg w-full md:w-auto">
              ...
              <button
                className="border-2 flex items-center gap-2 rounded-3xl py-1 px-5 font-semibold w-fit text-sm md:text-base"
              >
                Filter
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-8 px-12 mt-10 mb-16">
          {data.map((item) => (
            <div key={item.id} className="grid mb-3 min-w-[220px]">
              <div className="card border-2 max-w-[300px] border-gray-200 rounded-tl-[70px] rounded-br-[70px] px-18 shadow-lg hover:rounded-tr-[70px] hover:rounded-bl-[70px] hover:rounded-tl-none hover:rounded-br-none ">
                <img
                  src={item.image}
                  alt={item.title}
                  onClick={() => goToDetails(item.id)}
                  className="w-40 h-80 object-contain mb-4 hover:scale-105 transition-transform duration-300"
                />
              </div>
              <h3
                className="text-xl mt-1 text-black font-semibold mb-2"
                onClick={() => goToDetails(item.id)}
              >
